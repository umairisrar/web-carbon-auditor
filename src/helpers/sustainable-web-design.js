...
const fileSize = { GIGABYTE: 1000 * 1000 * 1000 };
 ...
 */
const energyPerByteByComponent = (bytes) => {
 ...
 */
const co2byComponent = (energyBycomponent, carbonIntensity = GLOBAL_INTENSITY) => {
 ...
 * @return {number} the total number in grams of CO2 equivalent emissions
 */
const perByte = (bytes, carbonIntensity = GLOBAL_INTENSITY) => {
    const energyBycomponent = energyPerByteByComponent(bytes);
    ...
    const co2ValuesbyComponent = co2byComponent(
        energyBycomponent,
        carbonIntensity
    );
 ...
 * @return {number} the total number in grams of CO2 equivalent emissions
 */
const perVisit = (bytes, carbonIntensity = GLOBAL_INTENSITY) => {
    const energyBycomponent = energyPerVisitByComponent(bytes);
 ...
 * @return {object} Object containing the energy in kilowatt hours, keyed by system component
 */
const energyPerVisitByComponent = (
    bytes,
 ...
 */
const energyPerVisit = (bytes) => {
    // fetch the values using the default caching assumptions
    // const energyValues = Object.values(energyPerVisitByComponent(bytes));
...
const emissionsPerVisitInGrams = (energyPerVisit, carbonintensity = GLOBAL_INTENSITY) => {
    return formatNumber(energyPerVisit * carbonintensity);
}

const annualEnergyInKwh = (energyPerVisit, monthlyVisitors = 1000) => {
    return energyPerVisit * monthlyVisitors * 12;
}

const annualEmissionsInGrams = (co2grams, monthlyVisitors = 1000) => {
    return co2grams * monthlyVisitors * 12;
}

const annualSegmentEnergy = (annualEnergy) => {
    return {
